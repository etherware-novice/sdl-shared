...
void getSurfacePixel( SDL_Surface *src, unsigned x, unsigned y, uint8_t *r, uint8_t *g, uint8_t *b, uint8_t *a )
{
	if(!src) return;
...
void getRenderPixel( SDL_Renderer *rend, unsigned x, unsigned y, uint8_t *r, uint8_t *g, uint8_t *b, uint8_t *a )
{
	if(!rend) return;
